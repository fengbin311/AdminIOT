...
class OneNetApi
{
    ...
    public function device_datapoints( $devIds)
    {
        if ( empty($devIds) || !is_array($devIds) ) 
           return false;
         ...
         $api = '/devices/datapoints?' . $params_str;
        return $this->_call($api);
    }
    ...
    public function devices_status($devIds)
    {
        if ( empty($devIds) || !is_array($devIds) ) 
           return false;
    ...
    public function cmd_send($dev_id, $qos, $timeout, $sms)
    {
        if (empty($dev_id) || empty($sms)) {
            return FALSE;
        }
        
         $api = "/cmds?device_id={$dev_id}&qos={$qos}&timeout={$timeout}";
    
        return $this->_call($api, 'POST',$sms);
    }
    

    public function cmd_histroy($dev_id,$page = 1, $page_size = 30)
    {
        if (empty($dev_id)) {
            return FALSE;
        }
        ...
        $params_str = http_build_query($params);
         ...
         $api = "/cmds/history/{$dev_id}?".$params_str;
    ...
    public function get_dev_cmd_status($cmd_uuid)
    {
        if (empty($cmd_uuid)) {
            return FALSE;
        }
    ...
    public function get_dev_cmd_resp($cmd_uuid)
    {
        if (empty($cmd_uuid)) {
            return FALSE;
        }
...
//         var_dump($url);
       ...
       // curl_setopt($ch,CURLOPT_PROXY,'127.0.0.1:8888');
        ...
        //SSL 单向认证， 客户端验证服务器端合法性。
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // 信任任何证书，不是CA机构颁布的也没关系 ，oneNet连接需要设置为false
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2); //是否检测服务器的域名与证书上的是否一致
