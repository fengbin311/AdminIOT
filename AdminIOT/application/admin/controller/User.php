...
class User extends Base
{
    ...
    public function add()
    {
        ...
        $this->assign('roles', AdminAuthGroups::all(['status' => 1]));
    ...
    public function edit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $roles      =  AdminAuthGroups::all(['status' => 1]);
        ...
        if ($this->request->isPost()) {
            ...
            if (false !== $user->save($user_update_data)) {
                if (false !== $user->profile->save($profile_update_data)) {
                    return $this->do_success('修改成功', 'admin/user/profile');
                }
    ...
    public function getProfile()
    {
      ...
      //  var_dump($user_id);
        ...
        $lists['errno'] = 0;
        $lists['user_name'] = $user->user_name;
        $lists['nick_name'] = $user->nick_name;
        
        $lists['mobile'] = $user->profile->mobile;
        $lists['email'] = $user->profile->email;
        $lists['city'] = $user->profile->city;
        $lists['description'] = $user->profile->description;
    ...
    {
        $logs       = new AdminLogs();
        $loginUserid = Session::get('user.user_id');
        
        //非超级管理员只能查看该用户的登录日志
        if($loginUserid != 1)
             $logs->where('user_id='.$loginUserid);
