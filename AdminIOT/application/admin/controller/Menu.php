...
class Menu extends Base
{
    ...
    public function index()
    {
        ...
        foreach ($result as $n => $r) {
            ...
            $result[$n]['str_manage'] .=
                '<a class="btn btn-danger btn-sm margin" data-toggle="modal" data-target="#modal" title="删除" 
                    onclick="del('. $r['menu_id'].')">
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            //添加url数据库唯一规则
            array_push($this->validate, ['url|url', 'unique:AdminMenus,url']);
        ...
        $requests  = Db::name('admin_request_type')->order('id asc')->select();
        $this->assign([
            'requests' => $requests,
            'selects'  => $selects
        ]);
    ...
    public function edit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            array_push($this->validate, ['url|url', 'unique:AdminMenus,url^menu_id']);
            //验证
            $result = $this->validate($post, $this->validate);
